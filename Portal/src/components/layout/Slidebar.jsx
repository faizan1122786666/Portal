...
/**
 * Component: Slidebar / Sidebar
 * Description: The primary navigation sidebar component for the application.
 * Why: To provide easy access to all main sections of the portal, with role-based navigation item visibility.
 */
    ...
    <>
      ...
      >
        ...
        <div className="h-16 flex items-center px-5 border-b border-[#1e3a5f] dark:border-gray-800/40 bg-[#2C5282] dark:bg-[#292c35] transition-colors duration-300">
          ...
          <button
            onClick={handleSlidebar}
            className="lg:hidden flex-shrink-0 text-white hover:bg-white/10 p-1 rounded-md transition-colors"
          >
          ...
          {menuItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `w-full flex items-center px-6 py-3 transition-all duration-200 border-l-4 ${
                    isActive
                ...
                <img
                  src={
                    userProfileImage.startsWith('http')
                      ? userProfileImage
                      : `http://localhost:3000/uploads/profile/${userProfileImage}`
                  }
                  alt="Profile"

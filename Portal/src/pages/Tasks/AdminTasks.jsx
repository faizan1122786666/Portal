...
//   // Stat cards — identical size/classes to AdminAttendance
//   const statCards = [
//     { label: 'Total Tasks',  value: stats.total,      sub: 'All Tasks', icon: <CgProfile size={20} className="text-white" /> },
//     { label: 'Awaiting',     value: stats.pending,     sub: 'Pending',   icon: <AiOutlineClockCircle size={20} className="text-white" /> },
//     { label: 'Working',      value: stats.inProgress,  sub: 'In Progress', icon: <FaRegCheckCircle size={20} className="text-white" /> },
//     { label: 'Finished',     value: stats.completed,   sub: 'Completed', icon: <RxCrossCircled size={20} className="text-white" /> },
//   ];
...
//       {/* Stat Cards — same size as AdminAttendance */}
//       <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-6">
...
//               <p className="text-[10px] sm:text-xs text-gray-400 dark:text-gray-500 mt-1">{card.label}</p>
//             </div>
//             <div className="bg-[#365F8D] dark:bg-[#2C5282] w-9 h-9 sm:w-10 sm:h-10 rounded-full flex items-center justify-center flex-shrink-0">
//               {card.icon}
//             </div>
//           </div>
//         ))}
...
//         <AddTaskModal
//           onClose={() => setIsModalOpen(false)}
//           onAdded={() => { setIsModalOpen(false); fetchTasks(); }}
//         />
//       )}
...
//         <EditTaskModal
//           task={editingTask}
//           onClose={() => { setEditingTask(null); setIsEditModalOpen(false); }}
//           onUpdated={() => { setEditingTask(null); setIsEditModalOpen(false); fetchTasks(); }}
//         />
//       )}
//     </div>
//   );
...
export default function AdminTasks({ setTitle }) {
  ...
  // ── Stat cards — identical classes to AdminAttendance ─────────────────────
  const statCards = [
    ...
    <div className="min-h-screen p-4 sm:p-6 lg:p-8 bg-gray-50/50 dark:bg-[#292c35]">
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right whitespace-nowrap">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        onClick={() => handleDelete(task._id)}
                        className="p-1.5 text-red-500 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors cursor-pointer"
                        title="Delete Task"
                      >
                        <AiOutlineDelete size={16} />
                      </button>
      ...
      {isEditModalOpen && editingTask && (
        <EditTaskModal
          ...
          onUpdated={() => { setIsEditModalOpen(false); setEditingTask(null); fetchTasks(); }}
        />
      )}
    </div>
  );
